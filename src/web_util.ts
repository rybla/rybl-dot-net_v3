...
> = (input) => async (ctx) => {
  try {
    const pageUrl = new URL(input.pageUrlString);
    const faviconUrl = new URL(
      `${pageUrl.protocol}//${pageUrl.hostname}/favicon.ico`,
    );
...
//     let html: JSDOM | undefined = undefined;
//     try {
//       html = new JSDOM(await response.text());
//     } catch (e: any) {
//       await Effect.tell(`Failed to parse HTML`)(ctx);
//       return undefined;
//     }
...
//       } catch (headError) {
//         await Effect.tell(
//           `HEAD request for /favicon.ico failed for ${pageUrl.origin}, trying GET`,
//         )(ctx);
//       }
